...
```
git clone https://github.com/HalinaPP/nodejs2022Q4-service
```
...
2. In the first task these endpoints operate only with **in-memory** (hardcoded) data, in the next tasks will be used a DB for it. 

3. An `application/json` format  is used for request and response body.

4. `User`'s password is excluded from server response.

5. When `Artist`, `Album` or `Track` are deleted, it's `id` is deleted from favorites (if was there) and references to it in other entities are become `null`. For example: `Artist` is deleted => this `artistId` in corresponding `Albums`'s and `Track`'s become `null` + this artist's `id` is deleted from favorites, same logic for `Album` and `Track`.
